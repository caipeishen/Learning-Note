...
　　
比如我创建一个项目，我一般会将GroupId设置为cn.cps，cn表示域为中国，cps是我个人姓名缩写，ArtifactId设置为proj，表示你这个项目的名称是proj，依照这个设置，在你创建Maven工程后，新建包的时候，包结构最好是cn.cps.proj打头的，如果有个StudentDao[Dao层的]，它的全路径就是cn.cps.proj.dao.StudentDao
```
