...
+ 有n位无符号数 就能表示2^n，2^8个数， [0, 2^n-1]，[0, 255]

  + -1：0也是数字，所以要减一，就像十进制最大的两位数99(10^2-1)一样

+ 有n位的有符号数 能表示2^(n-1)，2^(8-1)个数，[-2^(n-1), +2^(n-1)-1]，[-128 127]

  + (n-1)：本来8个数字组合，因为有个符号位，变成7个数字组合了
...
参考：[Spring事务不生效原因](https://zhuanlan.zhihu.com/p/570997764)



### Spring事务传播属性之REQUIRES_NEW

参考：[Spring事务传播属性之REQUIRES_NEW用法](https://www.jianshu.com/p/3e9267b025b2)



### Spring事务传播属性之REQUIRES_NESTED

参考：[PROPAGATION_NESTED的理解](https://blog.csdn.net/yanxin1213/article/details/100582643)  [NESTED需要try catch](https://blog.csdn.net/zmzmz_m/article/details/80198072)

+ requires_new 和nested 是事务独立，但是异常报给A方法后导致了A回滚，只有try_catch处理后才能体现requires_new 和nested的事务独立性
+ nested 作为事务嵌套，同时兼具了required，requires_new的特性。当外层事务回滚时，nested 与 required 同时内层全部进行回滚，当内层事务回滚时，nested 与 requires_new 不影响外层事务



### SpringCloud以及Nacos服务注册IP选择问题

参考：[SpringCloud以及Nacos服务注册IP选择问题](https://blog.csdn.net/Q1sjjj/article/details/118707056)
...
List<User> collect = userList.stream()
    .filter(c -> StrUtil.isNotBlank(c.getUserNameEn()) && StrUtil.isNotBlank(c.getUserNameCn()))
    .collect(Collectors.toMap(a -> a.getUserNameEn() + "_" + a.getUserCn(), b -> b, (p1, p2) -> p1))
    .values()
...
参考：[Maven package、install、deploy](https://blog.csdn.net/jam_yin/article/details/101777772/)
...
```java
private List<GoodsCategoryTreeVO> getGroupChildren(GoodsCategoryTreeVO root, List<GoodsCategoryTreeVO> all) {
    return all.stream()
        .filter(item -> item.getPid().equals(root.getId()))
        .peek(treeVO -> treeVO.setChildren(getGroupChildren(treeVO, all)))
        .sorted(Comparator.comparing(GoodsCategoryTreeVO::getIndex))
...
>参考：[浅析一致性hash和hash槽](https://blog.csdn.net/weixin_43001336/article/details/122834046)  [周阳讲解](https://www.bilibili.com/video/BV1gr4y1U7CY/?p=42)

+ 一致性hash解决了不用全部节点更换问题，扩容缩容只会影响数据前的一个节点。
+ 但一致性hash数据可能会分布bu均匀，数据倾斜，产生雪崩。造成不均匀的原因，是因为数据直接落在hash环上，服务器节点少且服务器节点离得近的时候，而hash环还是那么多，就会分布不均匀（虚拟节点可以解决，将真实节点计算多个哈希形成多个虚拟节点并放置到哈希环上，类似slot）。

![](./images/一致性hash.png)



+ hash槽就是解决数据分布不均匀问题，在数据和节点之间添加一个哈希槽(slot)。
+ slot=CRC16(key) % 16384，以槽为单位移动数据。如何分配槽是我们自己定义，可控，解决了分布不均匀。例如master1：[0-500] [1000-1500]
  + master1：[0-5460] 
  + master2：[5461-10922]
  + master3：[10923-16383]
